...
function readTrustedHostedBaseUrl(): string {
  ...
  if (!untrusted) throw new Error("PLAYWRIGHT_BASE_URL is required for hosted SRS acceptance.");
  let parsed: URL;
  try {
  ...
  if (
    ...
    throw new Error("Hosted SRS acceptance is restricted to this repository's Vercel project.");
  }
...
if (!process.env.HOSTED_ACCEPTANCE_RUN_ID || !process.env.HOSTED_FIXTURE_CONFIRMATION_FILE) {
  throw new Error("Hosted SRS acceptance must be started through its guarded Preview runner.");
}
...
export default defineConfig({
  ...
  forbidOnly: true,
  fullyParallel: false,
  outputDir: "test-results/hosted-srs",
  reporter: "list",
  retries: 0,
  testDir: "./e2e",
  testMatch: "**/hosted-srs.spec.ts",
  timeout: 240_000,
  use: {
