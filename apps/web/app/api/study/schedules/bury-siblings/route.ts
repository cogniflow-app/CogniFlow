...
const schema = z
  .object({
    buriedUntil: z.iso.datetime({ offset: true }),
    cardId: z.uuid(),
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success || new Date(parsed.data.buriedUntil) <= new Date())
    return context.applyCookies(
      apiError(422, {
        code: "INVALID_INPUT",
        message: "The sibling bury command is invalid.",
        retryable: false,
      }),
    );
  const { data, error } = await context.privileged.rpc("admin_bury_srs_siblings", {
    p_actor_account_id: context.accountId,
    p_auth_session_id: context.authSessionId,
    p_buried_until: parsed.data.buriedUntil,
  ...
  if (error || !data)
    return context.applyCookies(
      srsDatabaseError(error ?? {}, "The card’s siblings could not be buried."),
    );
