...
function readTrustedHostedBaseUrl(): string {
  ...
  if (!untrusted) {
    throw new Error("PLAYWRIGHT_BASE_URL is required for hosted portability acceptance.");
  }
  let parsed: URL;
  try {
  ...
  if (
    ...
    throw new Error(
      "Hosted portability acceptance is restricted to this repository's Vercel project.",
    );
  }
...
if (!process.env.HOSTED_ACCEPTANCE_RUN_ID || !process.env.HOSTED_FIXTURE_CONFIRMATION_FILE) {
  throw new Error("Hosted portability acceptance must use its guarded Preview runner.");
}
...
if (
  ...
  throw new Error("Hosted Playwright must not inherit provider or bypass credentials.");
}
...
export default defineConfig({
  ...
  forbidOnly: true,
  fullyParallel: false,
  outputDir: "test-results/hosted-portability",
  reporter: "list",
  retries: 0,
  testDir: "./e2e",
  testMatch: "**/hosted-portability.spec.ts",
  timeout: 360_000,
  use: {
